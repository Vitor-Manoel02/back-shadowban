import { getShadowBanStatus, verifyPageAndProfile, verifyPostAndHashtag } from '../services/usuario.service.js';
...
const sendverifyPageAndProfile = async (req, res) => {
  ...
  if(!userData.pageProfile) return res.status(500).json({
    message: "Conta não vinculada com uma página de negócios do facebook.",
  })
  if(!userData.instagramProfileId) return res.status(500).json({
...
const verifyRecentPostWithHashtag = async (req, res) => {
  ...
  if(!userData.hashtagId) return res.status(500).json({
